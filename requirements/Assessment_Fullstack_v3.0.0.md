...
Source: `Assessment_Fullstack_v3.0.0.pdf` (15 pages). The document title and footers identify version 2.3.1, despite the source filename.
...
Users must be able to view the details of an individual invoice by selecting an invoice from the Invoice List screen. Upon selection, the application shall navigate the user to the Invoice Details view, which must display comprehensive information relating to the selected invoice, including:
...
The frontend application must be implemented in ReactJS using TypeScript. The following requirements must be adhered to:
...
The backend service must be implemented using NestJS with TypeScript, and must expose a RESTful API. The application must be supported by a relational database, with PostgreSQL recommended as the preferred database system. The backend implementation should ensure a clean, modular architecture and support reliable data persistence and retrieval for all application features.
...
Expected response shape for `GET /invoices` (the ellipsis represents invoice records):
...
Overdue status behaviour: Overdue is a derived status — it is not persisted in the database. The backend computes it at read time:
...
The backend must implement authentication and authorization using JSON Web Tokens (JWT) to support stateless session management. The following requirements apply:
...
Unit testing is mandatory for the backend implementation. While no minimum test coverage threshold is enforced, candidates are expected to ensure adequate coverage of core functionality. The following requirements must be satisfied:
...
The backend API must be documented using Swagger/OpenAPI, generated via the @nestjs/swagger package. The following requirements apply:
...
Organize your project as a monorepo or two separate repos. Either is acceptable — document your choice in the README. Suggested monorepo structure:
...
The application must support environment-based configuration using .env files for all environment-specific settings. The following requirements apply:
...
Candidates must ensure that all submissions are completed and sent no later than the specified due date and time. All submission materials, including links to code repositories, must be sent to the designated submission contact provided below.
